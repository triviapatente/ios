//
//  BulletinChooser.swift
//  Trivia Patente
//
//  Created by Gabriel Ciulei on 16/03/2018.
//  Copyright © 2018 Terpin e Donadel. All rights reserved.
//
import BulletinBoard

open class ChooserBulletinItem: BulletinItem {
    ...
    public var actionHandler: ((ChooserBulletinItem) -> Void)? = nil
    ...
    public func makeArrangedSubviews() -> [UIView] {
        ...
        if let alternativeButtonTitle = self.alternativeButtonTitle {
            ...
            for alt in alternativeButtonTitle {
                let alternativeButton = interfaceFactory.makeAlternativeButton(title: alt)
                alternativeButton.layer.borderWidth = CGFloat(2)
                alternativeButton.layer.borderColor = Colors.primary.cgColor
                alternativeButton.layer.cornerRadius = CGFloat(12)
                alternativeButton.heightAnchor.constraint(equalToConstant: 45).isActive = true
                alternativeButton.setTitleColor(Colors.primary, for: .normal)
                ...
                self.alternativeButton!.append(alternativeButton)
            }
            
            if let actionButtonTitle = self.actionButtonTitle {
                
                let actionButton = interfaceFactory.makeAlternativeButton(title: actionButtonTitle)
                actionButton.backgroundColor = Colors.primary
                actionButton.layer.cornerRadius = CGFloat(12)
    ...
    public func tearDown() {
        ...
        if let btns = self.alternativeButton {
            for btn in btns {
